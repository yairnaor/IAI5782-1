...
def create():
    ...
    for i in range(3):
        board = board + [3 * [0]]
    # [the board, determines who is winner, human value, num of empty slots]
...
def checkSeq(s, r1, c1, r2, c2):
    ...
    # If no Xs (and at least 1 O) returns -1, Otherwise returns 0.
    ...
    for i in range(SIZE):  # summing the values in the seq.
        total += s[0][r1 + i * dr][c1 + i * dc]
    if total == COMPUTER * SIZE:
        return VIC
    if total == HUMAN * SIZE:
        return LOSS
    if 0 < total < COMPUTER:
        return -1
    if total > 0 and total % COMPUTER == 0:
        return 1
    return 0
